...
static int prio_parse_opt(struct qdisc_util *qu, int argc, char **argv, struct nlmsghdr *n)
{
	...
	while (argc > 0) {
        ...
        //test log
        syslog(LOG_ERR, "prio_parse_opt bands \n");
			if (pmap_mode)
				explain();
			NEXT_ARG();
        ...
        //test log
        syslog(LOG_ERR, "prio_parse_opt Illegal bands \n");
				return -1;
			}
		} else if (strcmp(*argv, "priomap") == 0) {
        //test log
        syslog(LOG_ERR, "prio_parse_opt priomap \n");
			if (pmap_mode) {
				fprintf(stderr, "Error: duplicate priomap\n");
        //test log
        syslog(LOG_ERR, "prio_parse_opt duplicate priomap \n");
				return -1;
			}
			pmap_mode = 1;
		} else if (strcmp(*argv, "multiqueue") == 0) {
        //test log
        syslog(LOG_ERR, "prio_parse_opt multiqueue \n");
			mq = 1;
		} else if (strcmp(*argv, "flow") == 0) {
        //test log
        syslog(LOG_ERR, "prio_parse_opt flow \n");
			NEXT_ARG();
			...
			{
        //test log
        syslog(LOG_ERR, "prio_parse_opt flow return\n");
				explain();
				return -1;
			}
		} else if (strcmp(*argv, "help") == 0) {
        //test log
        syslog(LOG_ERR, "prio_parse_opt help \n");
			explain();
			return -1;
		} else {
        //test log
        syslog(LOG_ERR, "prio_parse_opt else \n");
			unsigned band;
			if (!pmap_mode) {
				fprintf(stderr, "What is \"%s\"?\n", *argv);
				explain();
        //test log
        syslog(LOG_ERR, "prio_parse_opt else return %s \n",*argv);
				return -1;
			}
			if (get_unsigned(&band, *argv, 10)) {
				fprintf(stderr, "Illegal \"priomap\" element\n");
        //test log
        syslog(LOG_ERR, "prio_parse_opt else Illegal \n");
				return -1;
			}
			if (band > opt.bands) {
				fprintf(stderr, "\"priomap\" element is out of bands\n");
        //test log
        syslog(LOG_ERR, "prio_parse_opt else element is out of bands \n");
				return -1;
			}
			if (idx > TC_PRIO_MAX) {
				fprintf(stderr, "\"priomap\" index > TC_PRIO_MAX=%u\n", TC_PRIO_MAX);
        //test log
        syslog(LOG_ERR, "prio_parse_opt else TC_PRIO_MAX \n");
				return -1;
			}
